    ...
    */
    ...
    class URDFLoader {
        ...
        load(urdf, onComplete, onProgress, onError) {
            ...
            request.open('GET', urdfPath, true);
            ...
            request.onload = () => {
                ...
                if (request.status === 200 || request.status === 0) {
                    ...
                    const model = this.parse(request.responseText);
                    onComplete(model);
                    manager.itemEnd(urdfPath);

                } else {

                    const error = new Error(`URDFLoader: Failed to load URL '${urdfPath}' with status ${request.status}`);
                    if (onError) {
            ...
            request.onerror = () => {
                ...
                const error = new Error(`URDFLoader: Network error while attempting to load '${urdfPath}'`);
                if (onError) {
